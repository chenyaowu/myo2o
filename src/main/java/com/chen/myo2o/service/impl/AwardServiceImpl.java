...
public class AwardServiceImpl implements AwardService {
	...
	@Override
	@Transactional
	public AwardExecution addAward(Award award, CommonsMultipartFile thumbnail) throws IOException {
		...
		if (award != null && award.getShopId() != null) {
			...
			if (thumbnail != null) {
				...
				if (tempAward.getAwardImg() != null) {
					ImageUtil.deleteFile(tempAward.getAwardImg());
				}
				addThumbnail(award, thumbnail);
			}
			try {
	...
	private void addThumbnail(Award award, CommonsMultipartFile thumbnail) throws IOException {
		String dest = PathUtil.getShopImagePath(award.getShopId());
		String thumbnailAddr = ImageUtil.generateThumbnail(new ImageHolder(thumbnail.getName(),thumbnail.getInputStream()), dest);
